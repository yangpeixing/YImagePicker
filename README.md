## 关于YPXImagePicker
本库包含了微信的图片选择器、小红书多图剪裁选择器。采用切入式fragment代替传统的onActivityForResult，即调即用，减少开发成本！本Module与app主项目交互采用序列化接口方式实现，调用框架前需要实现交互层Presenter接口，比如指定固定的图片加载框架（Glide等）以及一些定制性UI弹窗等，从而实现高度解耦。本库已投入多个商业项目的使用，稳定可靠，持续迭代！
