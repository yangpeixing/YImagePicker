...
public class CropHelper {
    ...
    public static Bitmap cropViewToBitmap(CropImageView imageView, int cropWidth, int cropHeight) {
        Bitmap bitmap = imageView.getOriginalBitmap();
        if (bitmap == null) {
            return null;
        }
        ...
        //水平平移像素点
        float x = Math.abs(imageView.getTranslateX());
        //垂直平移像素点
        float y = Math.abs(imageView.getTranslateY());
        //缩放比例
        float scale = imageView.getScale();

        Log.e("cropViewToBitmap", "cropViewToBitmap: " + x + " " + y + " " + scale);
        //原图宽度(Glide压缩过的，Glide默认加载会减小大图的宽高)
        int bw = bitmap.getWidth();
        //原图高度(Glide压缩过的)
        int bh = bitmap.getHeight();
        ...
        float endY = 0;
        float cropRatio = (cropWidth * 1.00f / (cropHeight * 1.00f));
        if (imageView.getNewScaleType() == ImageView.ScaleType.CENTER_CROP) {//充满
            if (bRatio < cropRatio) {
                endW = bw / scale;
                endH = endW / cropRatio;
                endX = bw * x / (cropWidth * scale * 1.00f);
...
//            if (bw > bh || (bw == bh && cropRatio <= 1)) {//宽图，高填充剪裁区域（屏幕宽），宽度自适应
//                endH = bh / scale;
//                endW = cropRatio * endH;
//                endX = bh * x / (cropHeight * scale * 1.00f);
//                endY = bh * y / (cropHeight * scale * 1.00f);
//            } else if (bw < bh || cropRatio > 1) {//高图或者方形图，宽填充剪裁区域，高度自适应
//                endW = bw / scale;
//                endH = endW / cropRatio;
//                endX = bw * x / (cropWidth * scale * 1.00f);
//                endY = bw * y / (cropWidth * scale * 1.00f);
