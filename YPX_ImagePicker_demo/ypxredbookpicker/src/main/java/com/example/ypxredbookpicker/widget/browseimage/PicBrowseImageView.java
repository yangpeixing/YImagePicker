package com.example.ypxredbookpicker.widget.browseimage;
...
public class PicBrowseImageView extends ImageView {
    ...
    private void init() {
        ...
        linePaint.setStrokeWidth(ViewSizeUtils.dp(getContext(), 0.5f));
...
//        setMeasuredDimension(width, height);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (isShowLine) {
            canvas.drawLine(getWidth() / 3, 0, getWidth() / 3, getHeight(), linePaint);
            canvas.drawLine(getWidth() * 2 / 3, 0, getWidth() * 2 / 3, getHeight(), linePaint);
